...
/**
 * Confirma, no nível de endpoint (não só de {@code DashboardService}), que {@link
 * AccessDeniedException} lançada por {@code dashboard(projetoId)} vira {@code 403} — mesmo padrão
 * de {@code BoardControllerAccessDeniedIT} (achado de code review, agent QA, TASK-06.1).
 */
@WebMvcTest(controllers = DashboardController.class)
@Import(DashboardControllerAccessDeniedIT.SegurancaDeTeste.class)
class DashboardControllerAccessDeniedIT {
    ...
    void naoMembroDoProjeto_endpointRetorna403() throws Exception {
        ...
        when(dashboardService.dashboard(projetoId)).thenThrow(new AccessDeniedException("Acesso negado"));

        mockMvc
                .perform(get("/api/projetos/{projetoId}/dashboard", projetoId))
