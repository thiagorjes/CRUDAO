...
**Fluxo de notificação (RF-005):** ao alterar etapa/impedimento, o service resolve a lista de observadores (responsável + criador + `TarefaObservador`), cria uma `Notificacao` por observador e publica via `EventoBoardPublisher` em `/topic/notificacoes/{usuarioId}` (paralelo ao evento de board, mesmo `afterCommit`). Em ambiente multi-pod, o roteamento por usuário não pode depender de `convertAndSendToUser` isolado por pod — o backend registra o mapeamento `usuarioId → pod` (ou usa broadcast amplo do evento + filtro client-side por `usuarioId` no frontend, opção mais simples e escolhida nesta fase por não exigir componente novo) para garantir entrega independentemente de qual pod processou o `NOTIFY`.
