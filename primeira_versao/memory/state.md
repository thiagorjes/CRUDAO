...
- **Sincronização executada:** /spdd-sync — 2026-08-24
- **Divergências encontradas:** 1 (DEV-002, dimensão O — 4 tasks marcadas pendentes que já estavam implementadas/revisadas)
- **Resolvidas:** 1 | **Pendentes:** 0 | **Aceitas:** 0
- **Artefato:** docs/spdd/kanban-configuravel-deviations.md — canvas corrigido, TASK-04.2/01.3/02.3/06.1 marcadas `[x]`
...
- **Verificação:** TASK-02.3 — Frontend: excluir card pelo board (RF-002) — confirmada como já implementada (ícone de lixeira em `CardTarefa.tsx`, `excluirCard()`/`ModalConfirmacao` em `BoardApp.tsx`, tratamento 404 já presente) — verificação feita ao iniciar a TASK-03.1, que dependia dela

- **Task implementada:** TASK-03.1 — Testes E2E de criação e exclusão de card — 2026-08-24
- **Arquivos:** `frontend/e2e/criacao-exclusao-card.spec.ts` (novo, 8 cenários); `frontend/src/components/board/BoardApp.tsx` (fix de bug real achado pelo teste — ver achado técnico)
- **Testes:** 8 cenários E2E (Playwright) cobrindo RF-001/RF-002 — criar card pela UI, botão oculto sem `tarefa:gerenciar`, criação bloqueada em projeto finalizado, excluir card pela UI, lixeira oculta para dev-tier com `devPodeExcluirTarefa=false`, exclusão em tempo real (STOMP, ≤2s), botão desabilitado com workflow sem etapas, fluxo combinado criar+excluir — todos passando contra `docker compose up` real; suíte E2E completa revalidada (22/22, specs desta feature + kanban-configuravel), sem regressão; `tsc --noEmit`, `eslint` e `next build` limpos
- **Achado técnico (bug real, não só de teste):** `criar card pela UI` era flaky sob paralelismo — o card aparecia duplicado no board. Causa: `atualizarTarefaLocal` (handler do evento STOMP `TAREFA_CRIADA`) buscava a tarefa via `GET` quando não a encontrava no estado local e a inserida ao resolver, sem reconfirmar se ela já havia sido inserida nesse meio-tempo pela atualização direta de `criarCard()` (mesmo padrão de race já mitigado para `projetoId`, mas faltava para a existência do id). Corrigido: o `setEstado` da resolução do fetch agora também checa `a.tarefas.some(t => t.id === tarefa.id)` antes de inserir.
- **Nota de ambiente:** validação rodou contra stack local (`docker compose up -d --build`) trazida e derrubada nesta sessão, sem residual de banco entre execuções (fluxo limpo)
- **Code review (inline, agent QA general-purpose):** ✅ aprovado, 0 findings 🔴/🟡 bloqueantes; 1 nota 🟡 não-bloqueante (cenários "tempo real" desta task e de `board.spec.ts` testam propagação via API direta, não 2 UIs simultâneas — padrão já aceito anteriormente, não é regressão desta task)

- **Code review formal:** /code-review TASK-03.1 — **APROVADO** — 2026-08-24
- **Findings:** 0 críticos, 0 importantes, 2 sugestões 🔵 (S1 propagação de tempo real via API direta em vez de 2ª UI — mesmo padrão de `board.spec.ts`, não bloqueante; S2 `test-results/` versionado — **corrigido nesta revisão**, adicionado a `frontend/.gitignore` + removido do índice git)
- **Artefato:** `docs/checklists/criacao-card-board-TASK-03.1-review.md`
- **Guardrails extraídos:** G-CARD-01 (dedupe por id obrigatório em toda inserção de tarefa vinda de evento assíncrono — não só checagem de projeto); reforço de G-RT-01 (herdado, escopo ampliado por `TAREFA_EXCLUIDA`, não corrigido)
- **Canvas:** `docs/spdd/criacao-card-board-canvas.md` — dimensão S preenchida — **transitou para READY** (7/7 dimensões)

- **Sincronização executada:** /spdd-sync — 2026-08-24
- **Divergências encontradas:** 1 (DEV-001, dimensão O — todas as 5 tasks marcadas pendentes que já estavam implementadas/revisadas)
- **Resolvidas:** 1 | **Pendentes:** 0 | **Aceitas:** 0
- **Artefato:** docs/spdd/criacao-card-board-deviations.md — canvas corrigido, TASK-01.1/02.1/02.2/02.3/03.1 marcadas `[x]`
- **Feature criacao-card-board:** todas as 5 tasks do plano (01.1 a 03.1) concluídas e revisadas — pronta para merge
...
| docs/spdd/criacao-card-board-canvas.md | — | **READY** (7/7 dimensões preenchidas) |
| docs/checklists/criacao-card-board-TASK-03.1-review.md | 1.0 | ok — APROVADO |
