...
- [x] TASK-01.1 — Backend: migrar exclusão para soft-delete e publicar evento `TAREFA_EXCLUIDA`
- [x] TASK-02.1 [P] — Frontend: RBAC gating no BoardApp (permissão e toggle)
- [x] TASK-02.2 [P] — Frontend: criar card pelo board (RF-001)
- [x] TASK-02.3 [P] — Frontend: excluir card pelo board (RF-002)
- [x] TASK-03.1 — Testes E2E de criação e exclusão de card _(revisado por DEV-001 — /spdd-sync 2026-08-24)_
...
_Atualizado por: /code-review v1.0 — 2026-08-24_
> Decisões: —

**Restrições:**
- G-CARD-01 — dedupe obrigatório em qualquer inserção de tarefa vinda de evento assíncrono (STOMP/fetch): antes de adicionar ao estado local, checar `a.tarefas.some(t => t.id === tarefa.id)` — race entre atualização otimista direta e o handler do evento pode disparar inserção dupla (achado real na TASK-03.1, `BoardApp.tsx`).
- G-RT-01 (herdado de TASK-04.2/05.1) — subscription STOMP ainda não valida vínculo do usuário ao projeto; esta feature amplia o escopo (mais um tipo de evento, `TAREFA_EXCLUIDA`) sem corrigir. Débito não bloqueante.
- Artefatos de execução de teste (`test-results/`, `playwright-report/`) não devem ser versionados — adicionados ao `.gitignore` do frontend nesta revisão.

**O que NÃO fazer:**
- Não inserir tarefa recebida por evento assíncrono no estado do board sem antes reconfirmar projeto atual **e** ausência do id — ambos os guards são necessários (só o de projeto não evita duplicação).
- Não introduzir novo RBAC nesta feature além do gating de UI já padronizado (`GET /usuarios/me` + `GET /projetos/{id}/configuracao`) — toda autorização real permanece no backend (RNF-003/ADR-006).
