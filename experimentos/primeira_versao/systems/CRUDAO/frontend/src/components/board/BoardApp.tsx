    ...
    setEstado((atual) => {
      ...
      if (!existe) {
        ...
        api
          ...
          .then((tarefa) =>
            setEstado((a) => {
              // Reconfirma projeto e existência: entre o disparo do fetch e sua resolução, uma
              // atualização direta (ex.: `criarCard()`) pode já ter inserido a mesma tarefa —
              // sem esse guard, o card apareceria duplicado no board.
              if (!a || a.projeto.id !== evento.projetoId) return a;
              if (a.tarefas.some((t) => t.id === tarefa.id)) return a;
              return { ...a, tarefas: [...a.tarefas, tarefa] };
            }),
          )
          .catch(() => undefined);
        return atual;
