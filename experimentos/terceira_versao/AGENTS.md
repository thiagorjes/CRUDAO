...
> Gerado automaticamente por `init.py` em 2026-08-26. Não editar manualmente — reexecute `init.py`/`update.ps1` para regenerar.
...
- **/analyze** — Realiza análise cross-artefato entre PRD, TechSpec, artefatos de design e Tasks no processo SDD, detectando inconsistências, lacunas de cobertura, ambiguidades e contradições. Use ao validar a consistência entre artefatos, antes de gerar Tasks (gate --pre-tasks) ou antes de iniciar a implementação.
...
- **/prd** — Conduz entrevista estruturada de requisitos e gera PRD completo com RFs, RNFs, regras de negócio e critérios de aceite Gherkin (N cenários por RF, parametrizável no início do levantamento de RFs). Use no início de qualquer nova feature após /discovery (ou direto, se discovery não foi executado).
