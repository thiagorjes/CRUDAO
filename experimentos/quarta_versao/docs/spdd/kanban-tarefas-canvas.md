...
- **RBAC de papéis/usuários (pacote `com.crudao.kanban.papel`, contrato `contracts/papeis-permissoes.md`): todo write passa por `permissaoGuard.exigir(projetoId, "papel:administrar")` + `exigirProjetoAtivo`; GET de leitura exige só `membro`. Papel `admin` é `protegido` — NÃO pode ser editado, excluído nem ter toggle de permissão alterado (RN-006, 403); só o `adminGlobal` pode associá-lo a um usuário. Chave `admin` reservada na criação (422). Excluir papel com vínculo de usuário → 409. **RN-017:** o autor NÃO pode alterar `PapelPermissao` de um papel que ele próprio possui no projeto (403); `adminGlobal` isento; toda alteração grava `PapelPermissaoAuditoria` (autor, valor anterior/novo, data/hora). Busca de usuários para associação (`/usuarios/buscar`) é sempre escopada por projeto (`buscarNaoAssociados`), min 3 chars, top 20, só ativos e não-associados — nunca uma listagem global da base; nunca expõe `keycloakSub`/`adminGlobal`/`ativo`.**
