...
- **[P] com:** TASK-03.1, TASK-03.2, TASK-03.3, TASK-04.1, TASK-04.2, TASK-04.3, TASK-04.4, TASK-04.5 (não bloqueia mais o board — só bloqueia TASK-07.5)
...
- **O que deve ser feito:**
  ...
  - [x] Implementar CRUD de Raia (contrato `raias.md`).
  - [x] Preparar RN-005 (bloquear exclusão com tarefas ativas vinculadas): stub "sem tarefas ativas" (mesma decisão de 03.2). **Checagem real obrigatória em TASK-04.1.**
...
- **O que deve ser feito:**
  ...
  - [ ] Publicar pelo canal dedicado `NOTIFY notificacao_events`; cada pod retransmite via STOMP para `/topic/notificacoes/{usuarioId}`, conforme decisão revisada na TechSpec Seção 5.
...
- **O que deve ser feito:**
  - [ ] Lista de notificações não lidas, conectada a `/topic/notificacoes/{usuarioId}`; autorização da subscrição é validada no backend.
...
_Ordem de início recomendada:_
