...
_Atualizado em: 2026-09-01 (16h40)_
...
|---|---|---|---|---|---|
| kanban-tarefas | CRUDAO | 1.0 | 1.1 | 1.1 | Implementação concluída — EPIC 01–08 concluídos (falta apenas /code-review de 08.1/08.2 e merge) |
...
| docs/checklists/kanban-tarefas-ui-fidelidade-review.md | 1.0 | REPROVADO - telas vs protótipos (3 críticos, 7 importantes) |
...
| 2026-09-01 | **RBAC implementado (TL-09 papéis/permissões + TL-10 associação de usuários)** — antes não havia NENHUM endpoint no backend. Novo pacote `com.crudao.kanban.papel` (`PapelController`/`PapelService` + DTOs), migration `V8__papel_permissao_auditoria.sql` (`ON DELETE CASCADE` no `papel_id`), entidade `PapelPermissaoAuditoria`. Endpoints conforme `docs/techspec/kanban-tarefas/contracts/papeis-permissoes.md`: `GET/POST /api/projetos/{id}/papeis`, `PUT/DELETE /api/papeis/{id}`, `PUT /api/papeis/{id}/permissoes/{chave}`, `POST/DELETE /api/projetos/{id}/usuarios/{usuarioId}`, `GET /api/projetos/{id}/usuarios/buscar?q=` (query `buscarNaoAssociados` já existia no repo, faltava controller). Guardrails: chave `admin` reservada (422); papel protegido read-only (RN-006, 403); DELETE papel com vínculos (409); **RN-017** — autor não altera permissão de papel que ele próprio possui no projeto (403), auditado em `papel_permissao_auditoria`; associar papel `admin` exige `adminGlobal`. Frontend: `PapeisView` (matriz permissão×papel, toggle por célula, CRUD papel custom), `usuarios/page.tsx` (troca de papel via delete+associate, remoção, associação via autocomplete). Route handlers proxy. **Suíte `-P integration-tests`: 185 testes, 0 falhas, 0 erros** (V8 aplicada no `kanban_it`). Stack rebuildado; V8 confirmada no banco de runtime. |
| 2026-09-01 | **Remediação da review de fidelidade de UI (2 commits).** Bloco 1-4: `globals.css` em paridade com `_shared.css` (+classes faltantes: badge-tipo/error, skeleton, kpi-grid, bar-chart, drawer, sidebar-section-title); `DashboardShell` reconstruído (nav fixa Projetos/Dashboard/Admin + chip "Projeto ativo"); Board (TL-03) reescrito com `.swimlane/.column/.task-card` + drag-and-drop nativo com indicador de destino válido/inválido; `CreateCardModal` (TL-05) com Raia/Responsável; `ConfirmarExclusaoModal` novo (TL-06, substitui `window.confirm()`); detalhe da tarefa (TL-04) agora é drawer sobre o board. Bloco 5-6: `DashboardView` (TL-07) com KPIs + gráfico de barras; Admin (TL-08) com abas reais, `ProjetoAdminForm` corrigido (finalizar/reabrir não faziam nada — usavam contrato errado), nova página `admin/workflows/[workflowId]` (fecha o 404 relatado pelo usuário) com tabela de colunas/transições editável, `RaiasList` com exclusão funcionando. **Zero `className` Tailwind restante** em `app/`/`components/` (framework nunca esteve instalado). Correções de contrato reais encontradas ao reconstruir (iam além de CSS): `impedimento`/exclusão de tarefa exigiam `projetoId` no backend e nenhum route handler enviava (sempre 400); `PUT /tarefas/{id}` e `POST /tarefas/{id}/mover` retornam 204 mas os route handlers chamavam `.json()` no corpo vazio (sempre 500 mesmo com sucesso no backend); `TarefaDetalheResponse` não tinha nenhum campo que o frontend usava (etapaNome/raiaNome/responsavelNome/criadoPorNome/observadores) — estendido o DTO; `GET /tarefas/{id}/auditoria` devolvia a entidade JPA com associações lazy fora de sessão (`open-in-view=false`) — 500 garantido em produção — novo `TarefaAuditoriaResponse`; `POST /raias` exigia `ordem` e o frontend não enviava; **novo endpoint** `GET /api/projetos/{id}/usuarios` (reaproveitou query já existente no repositório, sem controller). **Pendências que não são bug de UI, são RBAC não implementado no backend:** papéis/permissões (TL-09, nenhum controller) e associar/remover/trocar papel de usuário (escrita de TL-10) — as páginas agora dizem isso explicitamente em vez de expor forms que sempre falhavam. `mvn compile+test-compile`: BUILD SUCCESS; testes unitários afetados 22/22; `next build`/`tsc`/`vitest` 8/8 sem erros. Stack rebuildado e saudável no compose. |
| 2026-09-01 | /code-review de fidelidade de telas vs `docs/design/kanban-tarefas/prototypes/`: **REPROVADO** (3 críticos, 7 importantes). C1: telas TASK-07.2+ (board, card, modal nova tarefa, detalhe da tarefa, dashboard, admin) estilizadas com classes Tailwind, mas Tailwind não está instalado/configurado → renderizam sem estilo, ignoram `globals.css`/`_shared.css`. C2: TL-06 (confirmação de exclusão) usa `window.confirm()` nativo. C3: TL-04 é página cheia, não drawer. I1–I6: board/card/modal/dashboard/admin/sidebar reimplementados fora da marcação dos protótipos; rota `admin/workflows/[workflowId]` (gerenciar etapas/transições, TL-08 "Colunas/Transições") e `admin/usuarios` (TL-10) **não existem** — nem página nem route handler. Relatório: `docs/checklists/kanban-tarefas-ui-fidelidade-review.md`. Telas fiéis: TL-01 (login); parcial: TL-02. |
| 2026-09-01 | Validação em browser: 2 defeitos do frontend corrigidos — (1) `app/page.tsx` (scaffold TASK-01.1) colidia com `app/(dashboard)/page.tsx` e servia a tela placeholder pós-login → removido (`60e0f4d`); (2) EPIC 07 sem UI de criação de projeto (RF-008) e `/projetos` só listava vínculos (adminGlobal não via projetos criados) → `app/api/projetos/route.ts` (proxy GET/POST), `components/projetos/NovoProjetoButton.tsx`, `projetos/page.tsx` com botão gated por `me.adminGlobal` + lista via `GET /api/projetos` (`1a82954`). Frontend rebuildado no compose. |
| 2026-09-01 | /implement TASK-08.2 concluído — **EPIC 08 concluído**: `logback-spring.xml` (RollingFileAppender `logs/kanban-backend.log`, `SizeBasedTriggeringPolicy` 5MB, `FixedWindowRollingPolicy` maxIndex 9 → 10 arquivos totais, console mantido); métricas já cobertas por `AbstractListenNotifyRelay` (`kanban.listener.reconnections` Counter, `kanban.listener.notify_to_stomp` Timer, por canal) e `metrics` já exposto — `/actuator/metrics` responde 403 sem auth por decisão de TASK-08.3 (só `/actuator/health/**` permitAll); `docs/runbooks/keycloak-indisponivel.md` (stub: contexto ADR-006, sintoma, verificação, ação, escalonamento N1/N2/N3). Boot verificado no compose: log file criado, backend `healthy`, `listenNotify` UP nos 2 canais. Task config/doc-only, sem teste novo. |
...
- **Tasks Concluídas:**
  ...
  - `TASK-08.2` — Observabilidade final: `logback-spring.xml` (rotação 5MB / 10 arquivos), métricas listener já expostas, runbook stub Keycloak (2026-09-01)
  - `TASK-08.3` — Dockerização de backend e frontend
- **Última Etapa:** TASK-08.2 concluída — `logback-spring.xml` (rotação 5MB, 10 arquivos), runbook `docs/runbooks/keycloak-indisponivel.md`, métricas listener confirmadas; boot verificado no compose. **EPIC 08 concluído — implementação da feature kanban-tarefas completa** (2026-09-01)
